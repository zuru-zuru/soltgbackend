...
public:
	...
	virtual void analyze(SourceUnit const& _sources);
...
protected:
	...
	void endVisit(Literal const& _literal) override {SMTEncoder::endVisit(_literal);}
	...
	/// Helpers.
	//@{
	virtual void resetSourceAnalysis();
	...
	virtual Predicate const* createSymbolicBlock(smtutil::SortPointer _sort, std::string const& _name, PredicateType _predType, ASTNode const* _node = nullptr, ContractDefinition const* _contractContext = nullptr);
	...
	virtual void addRule(smtutil::Expression const& _rule, std::string const& _ruleName);
