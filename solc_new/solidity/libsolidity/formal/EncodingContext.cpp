...
void EncodingContext::changeVariable(frontend::VariableDeclaration const& _decl) {
	m_changed_variables.insert(&_decl);
}

void EncodingContext::changeState() {
	if (m_state.stateIndex() > 1) {
...
void EncodingContext::resetChangedVariables() {
	std::cout << "Reset\n";
	m_changed_variables.clear();
	m_changed_state = false;
}

bool EncodingContext::changedValue(frontend::VariableDeclaration const& _decl) {
	if (m_changed_variables.find(&_decl) != m_changed_variables.end()) {
		newValue(_decl);
		return true;
	}
	return false;
...
bool EncodingContext::changedState() {
	return m_changed_state;
}
