...
{
	// std::cout << "chc analyze called" <<std::endl;
	...
	m_dependency_handler.printDependancyGraph();
	...
	// 	setCurrentBlock(*target.header);
	// 	connectBlocks(predicate(*target.header), predicate(*target.header));
...
{
	m_dependency_handler.setCurrentContract(&_contract);
...
{
	hasBranch = 0;
	hasLoopOrRecursion = 0;
	// Free functions need to be visited in the context of a contract.
	if (!m_currentContract)
		return false;

	m_dependency_handler.addFunctionToCurrentContract(&_function);
...
{
	if (hasBranch > 0) {
		m_dependency_handler.setFunctionHasBranch(&_function);
	}
	if (hasLoopOrRecursion > 0) {
		m_dependency_handler.setFunctionHasLoopOrRecursion(&_function);
	}
	// Free functions need to be visited in the context of a contract.
	if (!m_currentContract)
		return;
...
bool CHC::visit(IfStatement const& _if)
{
	...
	deltaBranch += hasBranch - hasBranchprev;
	deltaLoops += hasLoopOrRecursion - hasLoopOrRecursionPrev;
	hasLoopOrRecursion = hasLoopOrRecursionPrev;
	hasBranch = hasBranchprev;
	...
	deltaBranch += hasBranch - hasBranchprev;
	deltaLoops += hasLoopOrRecursion - hasLoopOrRecursionPrev;
	hasLoopOrRecursion = hasLoopOrRecursionPrev + deltaLoops;
	hasBranch = hasBranchprev + deltaBranch;
...
void CHC::endVisit(FunctionCall const& _funCall)
{
	...
	if (funDef) {
		m_dependency_handler.addEdge(m_currentFunction, funDef);
		hasBranch += m_dependency_handler.functionHasBranch(funDef);
		hasLoopOrRecursion += m_dependency_handler.functionHasLoopOrRecursion(funDef);
	}
...
void CHC::visitDeployment(FunctionCall const& _funCall)
{
	...
	if (contract->constructor()) {
		m_dependency_handler.addEdge(m_currentFunction, contract->constructor());
		hasBranch += m_dependency_handler.functionHasBranch(contract->constructor());
		hasLoopOrRecursion += m_dependency_handler.functionHasLoopOrRecursion(contract->constructor());
	}
...
)
{
	...
	if (_funDef)
	{
		if (m_currentFunction && !m_currentFunction->isConstructor()) {
			m_dependency_handler.addEdge(m_currentFunction, _funDef);
			hasBranch += m_dependency_handler.functionHasBranch(_funDef);
			hasLoopOrRecursion += m_dependency_handler.functionHasLoopOrRecursion(_funDef);
	...
	if (m_currentFunction)
		for (auto const* definedFunction: contractFunctions(*m_currentContract))
			if (!definedFunction->isConstructor() && definedFunction->isPublic()) {
				...
				m_dependency_handler.addEdge(m_currentFunction, definedFunction);
				hasBranch += m_dependency_handler.functionHasBranch(definedFunction);
				hasLoopOrRecursion += m_dependency_handler.functionHasLoopOrRecursion(definedFunction);
			}
...
void CHC::externalFunctionCallToTrustedCode(FunctionCall const& _funCall)
{
	...
	if (m_currentFunction && !m_currentFunction->isConstructor()) {
		...
		m_dependency_handler.addEdge(m_currentFunction, function);
		hasBranch += m_dependency_handler.functionHasBranch(function);
		hasLoopOrRecursion += m_dependency_handler.functionHasLoopOrRecursion(function);
	}
	else {
