...
namespace ufo
{
  ...
  {
    return (isNumericConst(a) || isOpX<TRUE>(a) || isOpX<FALSE>(a));
  }
        ...
        for (auto it2 = expClauses.begin(); it2 != expClauses.end(); ++it2){
          if (isOpX<GEQ>(*it2)){
            ...
            if ( e1l == e2l ){
              ...
              if (clean){
                if (!contains(tmp, it1)) tmp.push_back (it1);
                if (!contains(tmp, it2)) tmp.push_back (it2);
                break;
              }
            }
          }
...
//                 exp->right()));
  ...
  {
    if (!isNumericConst(k)) return NULL;
    ...
    {
      if (!isNumericConst(a->right())) return NULL;
      if (!isNumericConst(a->last())) return NULL;
...
//                errs () << "WARNING. Wrong symbol at " << *t << ".\n";
  ...
  static Expr mkQFla (Expr def, Expr var, bool forall = false)
  {
    ExprSet vars = {var};
    return mkQFla(def, vars, forall);
  }
...
/*      else if (isOpX<OR>(c))
  ...
  inline void pprint(Expr exp, int inden = 0, bool upper = true)
  {
    ...
    if (isOpX<FORALL>(exp) || isOpX<EXISTS>(exp))
    {
      ...
      outs () << ". ";
      if (upper) outs () << "\n";
      pprint(exp->arg(i), upper ? inden + 2 : 0, upper);
      outs () << "]";
      if (upper) outs() << "\n";
      return;
    }
    else if (isOpX<AND>(exp))
    {
      op = " &&";
      getConj(exp, flas);
      if (flas.empty()) flas = {mk<TRUE>(exp->getFactory())};
    }
    else if (isOpX<OR>(exp))
    {
      op = " ||";
    ...
    {
      outs () << string(inden, ' ') << "[! ";
      pprint(exp->left(), upper ? inden + 2 : 0, upper);
      outs () << "]";
      if (upper) outs() << "\n";
      return;
    }
    ...
    else if (isOpX<IMPL>(exp))
    {
      ...
      outs () << " => ";
      if (upper) outs () << '\n';
      pprint(exp->right(), upper ? inden + 2 : 0, false);
      return;
    }
    ...
    else
    {
      ...
      if (flas.size() > 0) outs () << "[";
      outs () << (upper ? '\n' : ' ');
      pprint(flas, upper ? inden + 2 : 0, op);
      outs () << string(inden, ' ');
      if (flas.size() > 0) outs () << (upper ? '\n' : ' ') << "]";
    }
    if (upper) outs() << "\n";
  }
}
